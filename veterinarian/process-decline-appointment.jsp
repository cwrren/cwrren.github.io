<%-- 
    Document   : reject-appointment
    Created on : Apr 26, 2021, 4:32:13 PM
    Author     : cwrre
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@page import="java.util.*, java.sql.*, vetclinic.Appointment"%>
<jsp:useBean id="database" class="vetclinic.DBConnection" scope="request"/>
<jsp:useBean id="appointmentDao" class="vetclinic.AppointmentDAO" scope="request"/>
<!DOCTYPE html>
<html lang="en">
...
<%
            ...
            List<Appointment> allAppointment = appointmentDao.retrieveApprovedAppointment();
%>
                                ...
                                <tbody>
                                  <%
                                        for (int i = 0; i<allAppointment.size(); i++){
                                          ...
                                          out.println("<tr>");
                                          out.println("<td>"+allAppointment.get(i).getAptID() + "</td>");
                                          out.println("<td>"+allAppointment.get(i).getSlotID()+"</td>");
                                          out.println("<td>"+allAppointment.get(i).getCustID()+"</td>");
                                          out.println("<td>"+allAppointment.get(i).getPetID()+"</td>");
                                          out.println("<td>"+allAppointment.get(i).getStatus()+"</td>");
                                          out.println("<td><a href=../ApproveAppointmentServlet?aptID="
                                                  + allAppointment.get(i).getAptID() + " onclick=\"return confirm"
                                                  + "('Are you sure you want to approve?')\">Accept</a>"
                                                  + "i" + "<a href=../DeclineAppointmentServlet?aptID="
                                                  + allAppointment.get(i).getAptID() + " onclick=\"return confirm"
                                                  + "('Are you sure you want to decline?')\">Decline</a>"
                                                  + "</td>");
                                          out.println("</tr>");
                                      }
                                    %>
